...
class University:
    ...
    def add_user(self, login, passwd, role):
        try:
            sql = "INSERT INTO users(login, password,role)" \
                      " VALUES(%s,%s,%s)"
            args = (login, passwd, role)
            conn = mysql.connector.connect(host=self.host,
                                               database=self.database,
                                               user=self.user,
                                               password=self.password)
...
class Group(University):
    ...
    def student_of_group(self,number_of_group):
        try:
            sql = "SELECT S.idStudent,P.first_name, P.last_name,G.number_of_group,S._starosta, G.kyrs from Groups G, student S, People P where G.idGroup = S.group and  G.number_of_group = %s and S.idStudent=P.idPeople;"
            conn = mysql.connector.connect(host=self.host,
                                           database=self.database,
                                           user=self.user,
                                           password=self.password)
...
class Student(People):
    ...
    def update_student(self,idStudent, first_name, second_name, last_name, d_of_b,telephone,
                       group,starosta,):  # изменение имени, фамилии и отчества в бд
        try:
            sql = "SET SQL_SAFE_UPDATES = 0;" \
                  "UPDATE People P,Student S SET P.first_name = %s,P.second_name = %s,P.last_name = %s,P.d_of_b = %s,P.telephone = %s,S.group = %s,S._starosta = %s" \
                  " WHERE P.idPeople=S.idStudent and S.idStudent= %s"
            conn = mysql.connector.connect(host=self.host,
                                           database=self.database,
                                           user=self.user,
                                           password=self.password)
            ...
            c = conn.cursor()
            args = (first_name, second_name, last_name,d_of_b,telephone,group,starosta, idStudent)

            for result in c.execute(sql, args, multi=True):
                conn.commit()
                return 'Студент успешно изменён'
        except Error as e:
            return e
...
if __name__ == '__main__':
    ...
    student.add_student('Лера','Акаева','Cергеевна', '28/04/1995' ,'891111','35000','-')
    ...
    student.update_student('1','Дмитрий','Анисов', 'Александрович','24/03/1996','89210010','1','-',)